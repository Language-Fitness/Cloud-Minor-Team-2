package integration

import (
	"ExerciseMicroservice/test/integration/helper"
	"ExerciseMicroservice/test/integration/requests"
	"fmt"
	"github.com/99designs/gqlgen/client"
	"github.com/stretchr/testify/assert"
	"testing"
)

var Token = "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJuMzNESXZyQUZ0b1JGQ1d2UTMyOF85bXpjeU5JbXptZ1NSNFVKM05rdEdRIn0..itOZMLAiMpArPe5bte0Aq3wogIEyZPVfpNR1bVte6cWx72nthy-sx4oW05REAWJumfOw3PjiBA66Zm_KN2QfYkcVw-VdtUtNj98pwfrINHgpymRiIkfim2-kAfmw3n8MmjzS2g0s2l0WtlEhIbR2XunJYuZsoYYTaALpmp1vk5Rq-qBNTCRet5CIg9A8ANCRJj3ql6zt4WMiNBHKG4skDA6mftB1NasmT2qA--aRQR_NKToRE7Sbom1Mw1FcZcMJ5HtuQUplsPB0qpFXWw9duUUStzVQVecvmoVT_4QCnu3i5URfIWM3jsm0eZLyFVY71GafK9q07lohi25nkEtSjg"

func TestResolver_CreateExercise(t *testing.T) {
	// Example test
	c := helper.CreateClient()

	t.Run("Create exercise", func(t *testing.T) {

		var resp struct {
			CreateExercise struct {
				ID       string
				ClassID  string `json:"class_Id"`
				ModuleID string `json:"module_id"`
				Name     string
				Question string
				Answers  []struct {
					Value   string
					Correct bool
				}
				Difficulty string
				MadeBy     string `json:"made_by"`
			}
		}
		// Call the resolver via the client and modify the context via functional options
		c.MustPost(`
            mutation($exerciseInput: ExerciseInput!) {
                CreateExercise(exercise: $exerciseInput) {
                    id
                    class_Id
                    module_id
                    name
                    question
                    answers {
                        value
                        correct
                    }
                    difficulty
                    made_by
                }
            }`,
			&resp,
			client.Var("exerciseInput", requests.GenerateExerciseInput()),
			helper.AddContext(Token),
		)

		// Perform assertions based on your expected results
		assert.NotEmpty(t, resp)
		fmt.Printf("%+v\n", resp)
		assert.NotEmpty(t, resp.CreateExercise.ID)
		assert.Equal(t, requests.ClassID, resp.CreateExercise.ClassID)
		assert.Equal(t, requests.ModuleID, resp.CreateExercise.ModuleID)
		assert.Equal(t, requests.Name, resp.CreateExercise.Name)
		assert.Equal(t, requests.Question, resp.CreateExercise.Question)
		assert.Equal(t, requests.Answers[0]["value"], resp.CreateExercise.Answers[0].Value)
		assert.Equal(t, requests.Answers[0]["correct"], resp.CreateExercise.Answers[0].Correct)
		assert.Equal(t, requests.Answers[1]["value"], resp.CreateExercise.Answers[1].Value)
		assert.Equal(t, requests.Answers[1]["correct"], resp.CreateExercise.Answers[1].Correct)
		assert.Equal(t, requests.Difficulty, resp.CreateExercise.Difficulty)
	})
}
