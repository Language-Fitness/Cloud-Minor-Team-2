...
type FakeExercise struct {
	...
	Answers     []*FakeAnswer       `faker:"-"` // faker:"-" means that this field will not be generated by faker
...
func GenerateRandomExercises(n int) []FakeExercise {
	...
	for i := 0; i < n; i++ {
		...
		exercise.Answers = generateFakeAnswers(5)
...
func generateFakeAnswers(numAnswers int) []*FakeAnswer {
	answers := make([]*FakeAnswer, 0, numAnswers)
	randomNr := rand.Intn(numAnswers)
