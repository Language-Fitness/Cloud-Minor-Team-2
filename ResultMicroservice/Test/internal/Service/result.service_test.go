...
func TestResultService_GetResultByExerciseId(t *testing.T) {
	mockRepo := new(mocks.MockResultRepository)
	mockValidator := new(mocks.MockValidator)
	mockPolicy := new(mocks.MockResultPolicy)
	service := &Service.ResultService{Validator: mockValidator, Repo: mockRepo, ResultPolicy: mockPolicy}

	mockPolicy.On("GetResultByExercise", mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(nil)

	mockValidator.On("GetErrors").Return([]string{})

	mockRepo.On("GetResultByExerciseId", mock.AnythingOfType("string")).Return(&mocks.MockResult, nil)

	result, err := service.GetResultByExerciseId(adminToken, "sample_exercise_id")
...
func TestResultService_GetResultByClassId(t *testing.T) {
	mockRepo := new(mocks.MockResultRepository)
	mockValidator := new(mocks.MockValidator)
	mockPolicy := new(mocks.MockResultPolicy)
	service := &Service.ResultService{Validator: mockValidator, Repo: mockRepo, ResultPolicy: mockPolicy}

	mockPolicy.On("GetResultsByClass", mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(nil)

	mockValidator.On("GetErrors").Return([]string{})

	mockRepo.On("GetResultByClassId", mock.AnythingOfType("string")).Return([]*model.Result{&mocks.MockResult}, nil)

	results, err := service.GetResultByClassId(adminToken, "sample_class_id")

	assert.Nil(t, err)
	assert.NotNil(t, results)
	assert.Len(t, results, 1)
	assert.Equal(t, "sample_result_id", results[0].ID)
...
func TestResultService_GetResultsByUserID(t *testing.T) {
	mockRepo := new(mocks.MockResultRepository)
	mockValidator := new(mocks.MockValidator)
	mockPolicy := new(mocks.MockResultPolicy)
	service := &Service.ResultService{Validator: mockValidator, Repo: mockRepo, ResultPolicy: mockPolicy}

	mockPolicy.On("GetResultsByUser", mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(nil)

	mockValidator.On("GetErrors").Return([]string{})

	mockRepo.On("GetResultsByUserID", mock.AnythingOfType("string")).Return([]*model.Result{&mocks.MockResult}, nil)

	results, err := service.GetResultsByUserID(adminToken, "sample_user_id")
...
func TestResultService_DeleteResultByClassID(t *testing.T) {
	mockRepo := new(mocks.MockResultRepository)
	mockValidator := new(mocks.MockValidator)
	mockPolicy := new(mocks.MockResultPolicy)
	service := &Service.ResultService{Validator: mockValidator, Repo: mockRepo, ResultPolicy: mockPolicy}

	expectedUUID := "sample_uuid"
	mockPolicy.On("DeleteResultByClass", mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(&expectedUUID, nil)

	mockValidator.On("GetErrors").Return([]string{})
	mockRepo.On("DeleteResultByClassAndUserID", mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(nil)

	err := service.DeleteResultByClassID("sample_bearer_token", "sample_class_id")

	// Assert that the expected methods were called and with the expected arguments
	mockPolicy.AssertExpectations(t)
	mockValidator.AssertExpectations(t)
	mockRepo.AssertExpectations(t)

	// Assert that the function under test returned no error
	assert.Nil(t, err)
}
