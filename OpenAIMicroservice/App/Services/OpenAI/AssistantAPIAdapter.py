...
class AssistantAPIAdapter:
    ...
    def generate_multiple_choice_questions(self, question_subject, question_level, amount_questions):
        try:
            ...
            logging.error(f"An error occurred in generate_multiple_choice_questions: {e}")
            AssistantAPIException(str(e))

        except Exception as e:
            logging.error(f"Unexpected error in generate_multiple_choice_questions: {e}")
    ...
    def read_multiple_choice_questions_from_file(self, file_data, filename):
        try:
            ...
            logging.error(f"An error occurred in read_multiple_choice_questions_from_file: {e}")
            AssistantAPIException(str(e))

        except Exception as e:
            logging.error(f"Unexpected error in read_multiple_choice_questions_from_file: {e}")
            raise Exception(str(e))
    ...
    def generate_explanation(self, question_subject, question_text, given_answer, correct_answer):
        try:
            ...
            logging.error(f"An error occurred in generate_explanation: {e}")
            AssistantAPIException(str(e))

        except Exception as e:
            logging.error(f"Unexpected error in generate_explanation: {e}")
            raise Exception(str(e))
    ...
    def decode_and_check_token(self, token):
        ...
        try:
            ...
            raise AssistantAPIException("Invalid token format.")
    ...
    def is_valid_generated_question_json(self, json_data_dict):
        ...
        if 'question' not in json_data_dict or not json_data_dict['question']:
            raise AssistantAPIException("question field not found!")
